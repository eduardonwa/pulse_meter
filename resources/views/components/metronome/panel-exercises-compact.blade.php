    ...
    >
        ...
        <div class="exercise-layout__info">
            <section class="summary">
                <input class="name" type="text" x-model="step.name" @click.stop
                ...
                <div class="exercise-menu" x-data="{ open: false }" @click.outside="open = false">
                    <span class="timer" x-text="getStepTimeLabel(step, index)"
                        :class="{ 
                    ...
                    <div class="exercise-actions">
                        ...
                        <div class="exercise-actions__menu" x-show="open" x-cloak>
                            <button type="button" class="button" data-type="icon-text" @click.stop="open = false; openEditStepModal(index);">
                                <x-heroicon-s-pencil /> Edit
                            </button>
    
                            <button type="button" class="button" data-type="icon-text" :disabled="steps.length <= 1" @click.stop=" open = false; openDeleteStepModal(index);">
                                <x-heroicon-s-trash /> Delete
                            </button>
                        </div>
                    </div>
                </div>
